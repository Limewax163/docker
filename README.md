  ...
  <pre>
{
    ...
    "default-address-pools":[{"base":"100.100.0.0/25","size":28}], - задается рабочая сетка докера. Докер будет нарезать подсети из пула 100.100.0.0/25 (128 Доступных адресов в сети) и для каждой подсети будет выделять максимум 16 адресов (из них 14 адресов для использования, 1 — адрес сети, 1 — широковещательный адрес) что в итоге позволит иметь 8 подсетей внутри сети 100.100.0.0/25
    "bip": "100.100.1.1/26", - задается адрес шлюза в сеть/из сети докера (не должен пересекаться с сетями выделяемыми для контейнеров из конфигурации выше)
