...
`docker-compose up` - запускает сценарий docker-compose.yaml (При добавлении ключа -d поднимет контейнеры в фоновом режиме)
...
`docker-compose logs` - покажет логи текущего docker-compose проекта (При добавлении ключа -f будет показывать все новые логи в текущем окне)
